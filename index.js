...
io.sockets.on('connection', (socket, opt) => {
  ...
  socket.on('join', function(roomId, fn) { // 조인
  ...
  socket.on('leave', function(roomId, fn) { // 떠나기
  ...
  socket.on('rooms', function(fn) { // 룸스 
    if (fn)
      fn(Object.keys(socket.rooms));
  });

  // data: {room: 'roomid', msg: 'msg 내용..'}
  socket.on('message', (data, fn) => { //메세지
  ...
  socket.on('message-for-one', (socketid, msg, fn) => { //귓속말
